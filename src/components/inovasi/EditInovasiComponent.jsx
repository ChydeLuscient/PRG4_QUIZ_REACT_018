...
import { getProdukById, updateProduk } from '../../services/InovasiService';
import { Link, useNavigate, useParams } from 'react-router-dom';

function EditInovasiComponent() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchInovasi = async () => {
      try {
        ...
        if (!id) {
          throw new Error('ID inovasi tidak valid');
        }

        console.log('🔄 Fetching inovasi with ID:', id);
        
        const response = await getProdukById(id);
        const inovasiData = response.data;
        
        console.log('✅ Inovasi data received:', inovasiData);

        setFormData({
          ino_judul: inovasiData.ino_judul || '',
          ino_deskripsi: inovasiData.ino_deskripsi || '',
          ino_kategori: inovasiData.ino_kategori || 'SS',
          ino_tanggalsubmit: inovasiData.ino_tanggalsubmit || '',
          ino_pengusul: inovasiData.ino_pengusul || '',
          ino_unit: inovasiData.ino_unit || '',
          ino_manfaat: inovasiData.ino_manfaat || '',
          ino_potensi_savings: inovasiData.ino_potensi_savings || ''
        });
        
      } catch (error) {
        console.error("❌ Error fetching inovasi:", error);
        setFetchError(error.message || "Gagal mengambil data inovasi.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('📦 Data to send:', updatedInovasi);
      
      const response = await updateProduk(id, updatedInovasi);
      console.log('✅ Update response:', response);
      
      setSuccessMessage('✅ Inovasi berhasil diperbarui! Mengalihkan...');
      ...
      setError(error.message || "Gagal memperbarui inovasi. Silakan coba lagi.");
    } finally {
  ...
  if (loading) {
    return (
      <div className="container mt-5 text-center">
        ...
        <h4 className="text-muted mt-3">Memuat data inovasi...</h4>
          ...
          <div className="card shadow-sm">
            <div className="card-header bg-warning text-dark">
              <div className="d-flex justify-content-between align-items-center">
                <h4 className="mb-0">✏️ Edit Inovasi</h4>
                <div>
...
export default EditInovasiComponent;
